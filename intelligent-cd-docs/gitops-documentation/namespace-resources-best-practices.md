...
- Remove Kubernetes defaults (API server injects these automatically):
    * progressDeadlineSeconds: 600 (default)
    * revisionHistoryLimit: 10 (default)
    * strategy.rollingUpdate.maxSurge: 25% and maxUnavailable: 25% (defaults)
...
- Remove Kubernetes defaults (API server injects these automatically):
    ...
    * ipFamilies: [IPv4] (default)
    * ipFamilyPolicy: SingleStack (default)
...
- Remove Kubernetes defaults:
    * wildcardPolicy: None (default)
